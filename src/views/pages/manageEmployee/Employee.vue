              ...
              <v-container>
                <v-col>
                  <v-row cols="12" sm="6" md="4">
                    <v-text-field v-model="editedItem.name" label="Tên NV"></v-text-field>
                  </v-row>
                  <v-row col="12">
...
const apiURL = common._API_URL + 'api/Employee/'
export default {
  data: () => ({
    dialog: false,
    dialogDelete: false,
  ...
  methods: {
    ...
    editItem(item) {
      ...
      console.log(this.employees.indexOf(item))
    ...
    save() {
      ...
      if (edtIdx > -1) {
        axios
          .put(`${apiURL}UpdateEmployee/`, edtItm)
          .then(res => {
            if (res.status === 200) {
              console.log('editedIndex:: ', edtIdx)
              console.log('this.editedItem:: ', edtItm)
              Object.assign(this.employees[edtIdx], edtItm)
            }
          })
          .catch(err => console.log(err))
      } else {
        axios
          .post(`${apiURL}InsertEmployee/`, edtItm)
          .then(res => {
            if (res.status === 200) {
              this.employees.push(edtItm)
            }
          })
          .catch(err => console.log(err))
      }
